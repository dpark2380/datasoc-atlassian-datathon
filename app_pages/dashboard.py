...
tab_overview, tab_context, tab_risk, tab_segments, tab_reliability, tab_robustness, tab_docs = st.tabs(
    ["Overview", "Business context", "Risk model", "Usage segments (ML)", "Data reliability",
...
# --- Business context ---------------------------------------------------------
with tab_context:
    st.subheader("Why account-level risk is worth catching, in Atlassian's own numbers")
    st.caption(
        "Every figure on this tab comes from a document Atlassian filed with the SEC. Nothing here is "
